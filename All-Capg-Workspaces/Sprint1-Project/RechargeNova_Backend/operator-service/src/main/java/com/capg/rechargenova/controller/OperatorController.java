...
public class OperatorController {
    ...
    @GetMapping("/count")
    public ResponseEntity<Long> countOperators() {
        return ResponseEntity.ok(operatorService.countOperators());
    }

    @GetMapping("/plans/count")
    public ResponseEntity<Long> countPlans() {
        return ResponseEntity.ok(operatorService.countPlans());
    }

    @GetMapping("/{operatorId}/plans/paginated")
    public ResponseEntity<org.springframework.data.domain.Page<PlanDto>> getPlansByOperator(
            @PathVariable Long operatorId,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        logger.info("API Call: Get plans for operator {} (Page: {}, Size: {})", operatorId, page, size);
        return ResponseEntity.ok(operatorService.getPlansByOperator(operatorId, page, size));
    }
}