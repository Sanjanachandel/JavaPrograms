...
class JwtAuthenticationFilterTest {
    ...
    @Test
    void testPublicGetPaths() {
        String[] paths = {"/operators", "/operators/count", "/operators/plans/count"};
    ...
    @Test
    void testTokenValidationError_Exception() {
        MockServerHttpRequest request = MockServerHttpRequest.get("/users/profile")
                .header(HttpHeaders.AUTHORIZATION, "Bearer error-token")
                .build();
        ServerWebExchange exchange = MockServerWebExchange.from(request);
        
        when(jwtUtil.validateToken("error-token")).thenThrow(new RuntimeException("JWT Error"));
