...
class CoverageTest {
    ...
    void testDTOs() {
        ...
        op.setId(1L); op.setName("name"); op.setType("GSM"); op.setCircle("Circle1");
        op.setPlans(Collections.emptyList());
        assertEquals(1L, op.getId());
        assertEquals("name", op.getName());
        assertEquals("GSM", op.getType());
        assertEquals("Circle1", op.getCircle());
        assertEquals(0, op.getPlans().size());

        // PlanDto
        PlanDto plan = new PlanDto();
        plan.setId(1L); plan.setOperatorId(1L); plan.setAmount(10.0); plan.setValidity("30"); plan.setDescription("desc");
    ...
    void testEntities() {
        ...
        o.setId(1L); o.setName("name"); o.setType("GSM"); o.setCircle("Circle1");
        o.setPlans(Collections.emptyList());
        assertEquals(1L, o.getId());
        assertEquals("name", o.getName());
        assertEquals("GSM", o.getType());
        assertEquals("Circle1", o.getCircle());
        assertEquals(0, o.getPlans().size());

        // Plan
        Plan p = new Plan();
        p.setId(1L); p.setAmount(100.0); p.setValidity("30"); p.setDescription("desc"); p.setOperator(new Operator());
    ...
    void testMain() {
        ...
        assertNotNull(app);
        // Invoke main with dummy args to get coverage
        OperatorServiceApplication.main(new String[]{"--server.port=0"});
    }
}
