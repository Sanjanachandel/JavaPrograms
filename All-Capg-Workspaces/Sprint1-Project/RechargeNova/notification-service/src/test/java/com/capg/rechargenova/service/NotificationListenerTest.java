...
class NotificationListenerTest {
    ...
    void testConsumePaymentEvent_Success() {
        PaymentEvent event = new PaymentEvent(1L, 1L, 1L, "SUCCESS");
        
        when(userFeignClient.getUserById(1L)).thenReturn(new UserResponse(1L, "name", "email", "123"));
        when(rechargeFeignClient.getRechargeById(1L)).thenReturn(new RechargeResponse(1L, 1L, 1L, 1L, "123", BigDecimal.TEN, "S"));
        when(operatorFeignClient.getOperatorById(1L)).thenReturn(new OperatorDto(1L, "op", "type", "circle"));
        when(operatorFeignClient.getPlanById(1L)).thenReturn(new PlanDto(1L, 1L, 10.0, "30", "d"));
    ...
    void testConsumePaymentEvent_FailedStatus() {
        PaymentEvent event = new PaymentEvent(1L, 1L, 1L, "FAILED");
